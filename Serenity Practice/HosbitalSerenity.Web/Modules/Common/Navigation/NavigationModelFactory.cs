...
public class NavigationModelFactory : INavigationModelFactory
{
    ...
    public INavigationModel Create()
    {
        ...
        var isImpersonating = httpContext?.User?.Claims?.Any(c => c.Type == "IsImpersonating" && c.Value == "true") ?? false;

        if (isImpersonating)
        {
            // Check if "Exit Impersonation" already exists in the navigation
            bool exists = model.Items.Any(item => item.Title == "Exit Impersonation" ||
                                            item.Url == ToAbsolute("~/Services/Administration/ImpersonateUser/ExitImpersonation"));

            if (!exists)
            {
                // Dynamically add "Exit Impersonation" item
                model.Items.Add(new NavigationItem
                {
                    Title = "Exit Impersonation",
                    Url = "javascript:confirmExitImpersonation()",
                    //Url = ToAbsolute("~/Services/Administration/ImpersonateUser/ExitImpersonation"),
